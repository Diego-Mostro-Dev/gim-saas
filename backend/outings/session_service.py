...
from .models import OutingSessionRecord
...
class SessionService:
    ...
    def record_session(enrollment, session_date, source="manual", schedule=None):
        ...
        with transaction.atomic():
            try:
                record = OutingSessionRecord.objects.create(
                    gym=enrollment.gym,
    ...
    def remove_session(enrollment, session_date):
        ...
        deleted, _ = OutingSessionRecord.objects.filter(
            enrollment=enrollment,
            date=session_date,
        ).delete()
        if deleted == 0:
    ...
    def renew_package(enrollment, additional_sessions):
        ...
        rate, and the package total recomputes accordingly.
        """
        if enrollment.modality != "package":
            raise SessionError("La inscripción no es de modalidad paquete.")
        ...
        update_fields = ["package_total_sessions", "active"]
