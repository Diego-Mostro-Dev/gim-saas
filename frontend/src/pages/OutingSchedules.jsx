...
import OutingScheduleCard from "../components/outings/OutingScheduleCard";
import OutingScheduleForm from "../components/outings/OutingScheduleForm";
import ConfirmModal from "../components/ui/ConfirmModal";
import { DAY_NAMES } from "../constants/days";

import { useOutingSchedules } from "../hooks/useOutingSchedules";
import { useGym } from "../hooks/useGym";
import { getInactiveOutingSchedules, updateOutingSchedule } from "../services/outingsEnrollments.service";
...
function OutingSchedules() {
  ...
  const [isPendingActivation, setIsPendingActivation] = useState(() => {
    ...
    try {
      const stored = JSON.parse(sessionStorage.getItem("pendingOutingActivation") || "[]");
      return stored.includes(Number(outingId));
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const data = await getInactiveOutingSchedules(outingId);
    ...
    try {
      if (editingSchedule) {
        ...
        if (isPendingActivation) {
          toast.success("Horario creado. Salida activada.");
          setIsPendingActivation(false);
          try {
            const stored = JSON.parse(sessionStorage.getItem("pendingOutingActivation") || "[]");
            const updated = stored.filter((id) => id !== Number(outingId));
            sessionStorage.setItem("pendingOutingActivation", JSON.stringify(updated));
          } catch {
            // silently fail
          }
        } else {
  ...
  async function handleReactivateSchedule(schedule) {
    try {
      await updateOutingSchedule(schedule.id, { active: true });
    ...
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-surface px-4 text-text-primary">
        <p className="text-text-secondary">
          {txt(gym, "staff.salidas.schedules.disabled")}
        </p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-surface px-4 pb-28 pt-6 text-text-primary">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-text-secondary">
              Gestión de horarios de la salida
            </p>
          </div>
        </div>
      ...
      {isPendingActivation && !hasActiveSchedules && (
        <div className="mb-4 rounded-xl border border-warning/20 bg-warning-bg p-4 text-sm text-warning-text dark:bg-warning/10 dark:text-warning">
          ...
          <p className="mt-1">Creá al menos un horario para activar la salida. También podés restaurar horarios anteriores desde la sección de inactivos.</p>
        </div>
      )}
          ...
          {showForm ? (
            ...
            <div>
              <p className="mb-4 font-medium text-text-primary">
                Esta salida no tiene horarios activos.
              </p>
            ...
            <div>
              <p className="mb-4 font-medium text-text-primary">
                Esta salida no tiene horarios. Creá nuevos horarios para que esté disponible para los miembros.
              </p>
              <button
                onClick={() => setShowForm(true)}
                className="inline-flex items-center gap-2 rounded-xl bg-blue-500 px-4 py-2 text-sm font-medium text-white transition hover:bg-blue-600"
...
export default OutingSchedules;