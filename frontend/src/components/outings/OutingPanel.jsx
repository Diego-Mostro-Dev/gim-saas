...
function OutingPanel({
  ...
  const enrolledCount = outing.enrolled_count ?? 0;
  const scheduleCount = outing.schedule_count ?? 0;
  const hasPrice = Number(outing.monthly_price) > 0;
  const priceLabel = hasPrice
    ? `$${Number(outing.monthly_price).toLocaleString()}/mes`
    : "Gratis";
    ...
    <div className="rounded-xl border border-border bg-surface-elevated p-4 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          <div
            className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-lg font-bold text-white ${getAvatarColor(outing.name)}`}
            aria-hidden="true"
          >
            {outing.name?.charAt(0)?.toUpperCase() || "S"}
          </div>
        ...
        <div className="flex shrink-0 items-center gap-2">
          <button
            onClick={() => navigate(`/outings/${outing.id}/schedules`)}
      ...
      <div className="mt-2 space-y-1 text-sm text-text-secondary">
        {outing.trainer_name && (
          <p className="inline-flex items-center gap-1.5">
            ...
            <span className="font-medium text-text-primary">{outing.trainer_name}</span>
          </p>
        )}
      ...
      <div className="mt-4 flex flex-col gap-2 sm:flex-row">
        <button
          onClick={() => onToggleActive(outing.id, !outing.active)}
          ...
          aria-label={outing.active ? "Desactivar salida" : "Activar salida"}
        >
          {outing.active ? <ToggleLeft size={14} /> : <Power size={14} />}
          {outing.active ? "Desactivar" : "Activar"}
        </button>
      </div>
    </div>
  );
